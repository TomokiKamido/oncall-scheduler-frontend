...
export const setRole = functions
  .region(region)
  .https
  .onCall(async (data: SetRoleRequest, context): Promise<SetRoleResponse> => {
    ...
    try {
      ...
      if (role === 'manager' && chiefOf && Array.isArray(chiefOf) && chiefOf.length > 0) {
        // 管理部署の検証
        const validDepartments = chiefOf.filter(dept => 
      ...
      try {
        const roleRequestData = {
          ...
          requestedBy: context.auth?.uid || 'system',
 ...
 * 本番環境では使用を避けることを推奨
 */
export const setAdminBatch = functions
  .region(region)
  .https
  .onCall(async (data: { uids: string[] }, context) => {
    const startTime = new Date();
    
    try {
      functions.logger.info('setAdminBatch 関数開始', { 
        uidsCount: data.uids?.length || 0,
        caller: context.auth?.uid || 'anonymous'
      });
      ...
      for (const uid of data.uids) {
        try {
          // setRole 関数を内部的に呼び出し
          const result = await (global as any).setRole({ uid, role: 'admin' }, context);
