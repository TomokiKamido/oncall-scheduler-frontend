...
exports.syncUserRoles = exports.getActualUserRoles = void 0;
 ...
 */
exports.getActualUserRoles = functions
    .region(region)
    ...
    if (callerRole !== 'admin') {
        throw new functions.https.HttpsError('permission-denied', 'Only admins can view all user roles');
    }
    try {
        // 全ユーザーのカスタムクレームを取得
        ...
        throw new functions.https.HttpsError('internal', 'Failed to fetch user roles');
    }
});
/**
 * 権限を同期するCallable関数
 */
exports.syncUserRoles = functions
    .region(region)
    .https.onCall(async (data, context) => {
    var _a;
    // 認証チェック
    ...
    const callerRole = context.auth.token.role || 'staff';
    if (callerRole !== 'admin') {
        throw new functions.https.HttpsError('permission-denied', 'Only admins can sync user roles');
    }
    try {
        ...
        for (const user of allUsers.users) {
            try {
                ...
                if (roleMapping[currentRole]) {
                    ...
                    await admin.auth().setCustomUserClaims(user.uid, Object.assign(Object.assign({}, currentClaims), { role: newRole, updatedAt: new Date().toISOString() }));
        ...
        for (const doc of usersSnapshot.docs) {
            try {
                ...
                const currentRole = ((_a = user.customClaims) === null || _a === void 0 ? void 0 : _a.role) || 'staff';
                ...
                if (targetRole !== targetFirestoreRole) {
                    await admin.auth().setCustomUserClaims(uid, Object.assign(Object.assign({}, user.customClaims), { role: targetFirestoreRole, department: userData.department || '', updatedAt: new Date().toISOString() }));
        ...
        throw new functions.https.HttpsError('internal', 'Failed to sync user roles');
    }
});
//# sourceMappingURL=roleSyncFunctions.js.map